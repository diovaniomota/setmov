...
class SetmovSupabaseUser extends BaseAuthUser {
  SetmovSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> setmovSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SetmovSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
