...
const String oldSupabaseServiceRoleKey = 'SUA_SERVICE_ROLE_KEY_ANTIGA_DO_PROJETO_ATUAL';
...
const String newSupabaseServiceRoleKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh3a2tyeWxucXlvZXJwYWl1amZxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MDkwMjUxMywiZXhwIjoyMDc2NDc4NTEzfQ.GOOFG3rHOBS3JY0a-unZoRrdRj_oymVz4bmYgxUc87k';
// --------------------
  ...
  try {
    ...
    for (final bucket in buckets) {
      ...
      try {
        ...
        await newClient.storage.createBucket(bucketId,
            const BucketOptions(public: true)); // Forçando public por padrão para evitar problemas, ajuste se necessário
      }

      // 3. Listar e Migrar Arquivos
      print('   📂 Listando arquivos...');
      // List all files recursively
      final files = await oldClient.storage.from(bucketId).list(path: '', searchOptions: const SearchOptions(limit: 5000));
    ...
    print('Lembre-se de configurar as políticas de acesso (Policies) no Storage do novo projeto conforme necessário.');
...
Future<void> _processDirectory(
    SupabaseClient oldClient, SupabaseClient newClient, String bucketId, String path) async {
  ...
  try {
    ...
    for (final obj in objects) {
        ...
        // Vamos tentar baixar.
        try {
           final fileBytes = await oldClient.storage.from(bucketId).download(fullPath);
